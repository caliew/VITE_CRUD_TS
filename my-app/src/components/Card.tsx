          ...
          <div className='flex flex-col flex-wrap font-Roboto text-lg font-extralight justify-center items-center'>
            {RsltKEY.map((key, index) => (
              <div key={index} className="flex justify-center items-center">
                ...
                {GetLEDDisplay({ id: sensorId, reading: reading[key], sensorType })}<span className="px-1" />
                <div className='font-extralight pb-10'>{GetSensorUNIT(sensorType,key,unitSystem)}</div>
              </div>
            ))}
            {false && RsltKEY.map((key, index) => (
              <SimpleGauge key={index} value={reading[key]} className='' name={name} min={0} max={100} unit={GetSensorUNIT(sensorType,unitSystem)} />
            ))}
          </div>
        );
      }
