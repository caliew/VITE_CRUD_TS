...
const getPRESSFactor = (UNITSYSTEM: any) => {
  return typeof UNITSYSTEM === "undefined"
    ? 1.0
    : UNITSYSTEM.toUpperCase() === "GFORCE"
    ? 1.0197e-5
    : UNITSYSTEM.toUpperCase() === "BAR"
    ? 1.0e-5
    : 1.0;
};
// -------------------------
function hexToDecimal(hex: any) {
  return (
    (parseInt(hex.slice(0, 2), 16) * 256 + parseInt(hex.slice(2, 4), 16)) /
    10000
  );
}
...
function calculateCurrent(HEX: string, CTRATIO: any) {
  let _CURR =
    typeof CTRATIO === "undefined"
      ? Math.abs(HEXTOINT(HEX) / 100.0)
      : Math.abs(hexToDecimal(HEX));
  _CURR = _CURR * 1.0;
  let objResult: any = {
